...
## ~~The Assignment kill criterion 정의~~ ✅ 완료 (2026-06-20)
- **Done:** `docs/design/agentops-gateway-design.md` "Target User & Narrowest Wedge > Wedge Kill Criterion"에 작성. 중단/재검토 착수/기본값 3기준.
...
- **Ready:** `scripts/specs_to_issues.sh` — 이슈 7개 생성 + 각 spec 메타에 역링크(멱등). `gh` 설치+`gh auth login` 후 `bash scripts/specs_to_issues.sh` 한 번이면 완료.
- **Blocker:** 이 환경에 `gh` 미설치 + 인증은 수동(브라우저). specs는 YAML frontmatter가 아니라 상단 blockquote 메타에 링크됨.
