...
- [x] **T12 (P1, CC ~30분)** — circuit breaker 파드별 학습 실측 + 기각 근거 (§4) ✅ 2026-09-06
  ...
  - **선행 확인 결과 — 회로는 클러스터에서 켜져 있지도 않았다.** `k8s/base/`·`docker-compose.yml` 어디에도 `GATEWAY_CIRCUIT_THRESHOLD`가 없다. 재현하려면 먼저 켜야 한다(`kubectl set env ... THRESHOLD=2 COOLDOWN=15`). **`k8s/base/`는 그대로 뒀다** — 기본 비활성은 stretch 기능의 의도된 설계고, 켜는 것은 env 한 줄이다. 다만 **성공기준 #7(파드 삭제 → 회로 open을 Grafana에서 관측)은 이 값 없이는 성립하지 않는다** — 3주차 관측 스택에서 결정.
  - **결과 — 갈림을 실측했다. 그리고 그 갈림이 옳다.** `replicas: 3` + stateless에서 `ops-server`를 죽이고 **파드 하나에만** 실패를 먹였다(port-forward 직결). 파드별 `tools/list`의 `ops` 노출: 기준선 `있음/있음/있음` → 백엔드 사망 직후에도 `있음/있음/있음`(**아직 아무도 모른다**) → 실패 3회 후 `없음/있음/있음` → 복구+cooldown 후 probe 1회로 `있음/있음/있음`(그 파드만 스스로 회복).
  - **에이전트가 겪는 모습:** 같은 클라이언트가 같은 순간 인그레스로 `tools/list`를 12번 물으면 **ops 보임 8 / 안 보임 4** — 파드 3개 중 1개만 open인 상태와 정확히 일치한다.
  - **기각 근거 —** 회로가 지키는 것은 백엔드가 아니라 **그 파드의 연결**이다(`upstream.Backend._session`이 파드마다 별개). 공유 상태를 넣으면 양방향으로 틀린다: A의 깨진 연결이 멀쩡한 B·C를 차단하고, 반대로 B·C의 성공이 A의 죽은 연결을 계속 열어 둬 fail-fast를 없앤다. 갈림은 "아직 모르는 파드가 있다"는 뜻이지 틀린 상태가 아니며, 트래픽이 균등하게 오면 저절로 수렴한다.
  - **또 같은 함정에 걸렸다(T7의 §9).** `ops-server`를 `replicas: 0`으로 줄이고 `rollout status`가 즉시 성공을 반환하기에 "사망"으로 알고 호출했더니 결과가 전부 `OK`였다 — 종료 중인 파드가 멀쩡히 응답 중이었다. `replicas: 0`에서는 기다릴 새 파드가 없어 이 함정이 특히 조용하다. `settle()`로 파드가 실제로 사라질 때까지 기다려 해결.
- [x] **T9 (P1, CC ~1시간)** — findings 문서 완성 ✅ 2026-09-06
  ...
  - Verify: 성공기준 #2
  - **결과 — 세 요구 다 반영, 둘은 예상보다 크게.**
    - ① "안 고침"은 2종이 아니라 **4종**이 됐다. circuit breaker·ticket-server에 더해 **audit(T11)·rate limit(T7)도 안 고치는 것이 결론**이었다. 문서 첫 문장이 이걸 정면으로 내세운다 — "넷 다 고치지 않기로 끝났다, 서로 다른 네 가지 이유로." 근거가 넷 다 다르다는 게 이 문서의 값이다.
    - ② §1-A 결론에 **"B의 가장 중요한 한계"** 로 승격해 넣었다(§5에만 있으면 세션 전략을 읽는 사람이 못 본다).
